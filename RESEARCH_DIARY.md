...
提交并推送后，GitHub Pages 会自动生成文章页，首页按日期从新到旧展示标题、摘要和日期，`/research-diary/archive/` 按年份归档。`category` 和 `tags` 用于 Tags 页面，`reading_time` 用于显示预计阅读分钟数，三者均可省略。文件名中的英文短名会成为文章网址的一部分。
